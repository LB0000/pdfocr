...
function DocumentDetail({ documentId }) {
    ...
    const { getDocument, updateDocument, deleteDocument } = (0, DocumentContext_1.useDocuments)();
    const { templates, fetchTemplates } = (0, TemplateContext_1.useTemplates)();
      ...
      <div className="p-6 border-b">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex space-x-2">
            <button onClick={() => router.push('/documents')} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
              一覧に戻る
            </button>
            {!isEditing ? (<>
                ...
                <button onClick={handleUpdate} className="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" disabled={isLoading}>
                  保存
                </button>
              </>)}
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            ...
            <div className="space-y-4">
              ...
              {document.confidenceScore > 0 && (<div>
            ...
            <div className="space-y-4">
              {isEditing ? (<>
                  ...
                  <div>
                    ...
                    <select id="template" value={templateId} onChange={(e) => setTemplateId(e.target.value)} className="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
              ...
              {document.status === 'completed' && (<div>
                  <p className="text-sm font-medium text-gray-500">OCR結果</p>
                  {document.ocrResult ? (<div className="p-3 mt-1 overflow-auto text-sm text-gray-900 bg-gray-50 rounded-md max-h-60">
                      <pre>{typeof document.ocrResult === 'string' ? document.ocrResult : JSON.stringify(document.ocrResult, null, 2)}</pre>
                    </div>) : (<p className="mt-1 text-sm text-gray-500">OCR結果はありません</p>)}
                </div>)}
            </div>
          </div>
        </div>
